...
const AddFarmModal = ({ isOpen, onClose, handleRefetch }: any) => {
  ...
  const ownerList: any[] =
    (isSuperAdmin ? (allUsers as any) : (farmUsers as any))?.data?.data ?? [];
  const usersLoading = isSuperAdmin ? usersLoadingAll : usersLoadingFarm;
  const usersError = isSuperAdmin ? null : usersErrorFarm;
