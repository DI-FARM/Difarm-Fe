...
import { useUsers, useFetchUsers, isLoggedIn, activateAccount } from '@/hooks/api/auth';
...
import { MagnifyingGlassIcon, FunnelIcon, EnvelopeIcon, PhoneIcon, DocumentTextIcon, XMarkIcon } from '@heroicons/react/24/outline';
...
const Users = () => {
    ...
    const { users: farmUsers, loading: farmLoading, refetch, deleteUser } = useUsers();
    ...
    const loading = isSuperAdmin ? allLoading : farmLoading;
    ...
    const columns: TableColumnV2<any>[] = [
        ...
        { title: 'Role', accessor: 'role', render: (row: any) => <p>{row?.account?.role}</p> },
        {
            title: 'Status',
            accessor: 'status',
            render: (row: any) => (
        ...
        },
        {
            ...
            render: (row: any) => (
                <div className="flex gap-2 justify-center items-center">
                    <button type="button" onClick={() => navigate(`/account/users/detail/${row?.id}`)} className="text-primary">
                        View
                    </button>
                    {hasFullUsersUI && isSuperAdmin && !row?.account?.status && (
                        <button
                            type="button"
                    ...
                    <button type="button" onClick={() => { setSelectedUser(row); setIsUpdateModalOpen(true); }}>
                        <IconEdit className="text-primary" />
                    </button>
                    <button type="button" onClick={() => { setSelectedUser(row); setIsDeleteModalOpen(true); }}>
            ...
            <div className="p-4">
                ...
                <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
                    <div>
                        ...
                        <h1 className="text-2xl font-bold text-gray-900 dark:text-white mt-1">Users</h1>
                    </div>
                    <div className="flex items-center gap-2">
                        <button type="button" onClick={() => setIsAddModalOpen(true)} className="btn btn-primary inline-flex items-center gap-2">
                            <IconPlus className="w-5 h-5" /> Add New User
                        </button>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                    {['', 'ADMIN', 'MANAGER', 'VETERINARIAN'].map((r) => (
                        <button
                            key={r || 'all'}
                            type="button"
                            ...
                            >
                                <div className="p-4">
                                    ...
                                    <div className="mt-3 flex flex-wrap gap-1.5">
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => { setSelectedAccountIdForLogs(u.account?.id ?? null); }}
